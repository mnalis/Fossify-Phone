...
## Summary
This is unofficial fork of Fossify phone from https://www.github.com/FossifyOrg/Phone
...
4.2. Generate signing key
     `keytool -genkeypair -v -keystore release.keystore -alias release -keyalg RSA -keysize 4096 -validity 10000`

4.3. On your GitHub fork of Fossify-Phone settings, under "Security", click "Secrets and Variables"
     and then "Actions" and then under "Secrets" tab setup "Repository
     secrets" by clicking 4 times on "New repository secret", ad add:

     - RELEASE_KEYSTORE_BASE64: (output from `base64 -w 0 release.keystore`)
     - SIGNING_KEY_ALIAS: release (because of `-alias release` in keytool)
...
4.5 In your GitHub fork of Fossify-Phone, click "Actions", "Build signed release apk", "Run workflow",
...
4.7. Download the `.zip` to your phone, extract `.apk` from it and install it



## Install and configure the app

5. After installing (via instructions above, or simple download from
   https://github.com/mnalis/Fossify-Phone/releases/), you need to setup the app on the phone
   as follows:
...
5.2. In android "Choose default apps", you must make the Fossify Phone app the default for both
     ...
     adb shell settings get secure dialer_default_application # returned "null" for me, which is apparently OK
...
5.4. Reboot the phone

5.5. Test receiving calls, making calls, and getting notifications for missed calls
